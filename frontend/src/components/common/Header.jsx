...
export default function Header({ title, subtitle, setMobileOpen }) {
  ...
  const notifications = [
    { id: 1, title: 'Portal Ready', desc: 'Secure examination services active.', time: 'Just now' },
    { id: 2, title: 'New Exam Scheduled', desc: 'Data Structures test on 10th Aug.', time: '1 hr ago' },
  ];

  return (
    <header className="bg-white border-b border-emerald-100/80 sticky top-0 z-30 px-4 sm:px-6 py-3 flex items-center justify-between shadow-2xs">
      ...
      <div className="flex items-center space-x-3">
        ...
        <div className="hidden md:flex items-center relative">
          ...
          <input
            ...
            className="pl-9 pr-3 py-1.5 text-xs bg-slate-50 border border-slate-200 text-slate-800 placeholder-slate-400 rounded-lg focus:outline-hidden focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 w-48 lg:w-60 transition-all"
          />
        </div>
          ...
          {notificationsOpen && (
            <div className="absolute right-0 mt-2 w-72 bg-white rounded-xl shadow-lg border border-slate-200 p-3 z-50 text-slate-800">
              <div className="flex items-center justify-between border-b border-slate-100 pb-2 mb-2">
                ...
                <span className="text-[10px] text-emerald-600 font-semibold cursor-pointer hover:underline">Mark as read</span>
              </div>
              <div className="space-y-2">
                {notifications.map((n) => (
                  <div key={n.id} className="p-2 bg-[#F9FAF9] border border-emerald-100 rounded-lg text-xs space-y-0.5">
        ...
        <div className="flex items-center space-x-2.5 pl-2 border-l border-slate-200">
          ...
          <div className="hidden sm:block text-left">
            ...
            <div className="text-[10px] text-emerald-700 font-semibold capitalize">{role} Account</div>
          </div>
        </div>
      </div>
    </header>
